...
import { PrismaClient, type Habit, type JournalEntry, type SharedHabit } from '@prisma/client';
...
export const load = (async ({ locals }) => {
	...
	if (user) {
		...
		interface HabitWithShared extends Habit {
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			SharedHabit: any[];
			shared?: Shared[];
		}
		...
		const shared_habits: HabitWithShared[] | null = await prisma.habit.findMany({
			where: {
				user_id: user.userId
			},
			include: {
